...
2026-02-20 A/B Test Conclusion:
  ...
  - CAGR: +21.15% vs +11.47%  |  Max DD: -0.86% vs -2.62%
  - Profit Factor: 4.93 vs 1.41  |  Sharpe: 1.85 vs 0.82
  - Trades: 34/year vs 134/year

Production Config: Pure Momentum Breakout (15m context,
engulfing + 3-bar rising ADX/vol confirmation), MR disabled.
"""
import logging
import pandas as pd
import numpy as np
...
class AdaptiveStrategy(BaseStrategy):
    ...
    def on_candle_close(self, candle_manager, active_positions=None):
        ...
        if regime == "RANGING":
            ...
            # ADX must be rising for 3 consecutive bars
            adx_rising = False
            if len(adx_df) >= 3:
